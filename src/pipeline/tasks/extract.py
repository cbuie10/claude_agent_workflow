...
@task(name="extract_well_transfers_data", retries=2, retry_delay_seconds=10)
def extract_well_transfers_data(xlsx_url: str) -> list[tuple]:
    """Fetch Oklahoma Corporation Commission Well Transfers Excel data.

    Downloads .xlsx file, parses with openpyxl, and returns list of row tuples.
    The file is small (~943 rows), so timeout is set to 60 seconds.
    """
    ...
    # Convert to list of tuples, skip header row
    rows = []
